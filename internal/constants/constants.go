...
const (
	...
	SysextServiceDirOverride     = "/etc/systemd/system/systemd-sysext.service.d"
	SysextTimeoutServiceFile     = "timeout.conf"
	SysextImagePolicyServiceFile = "image-policy.conf"
	SysextDefaultPolicy          = "--image-policy=\"root=verity+signed+absent:usr=verity+signed+absent\""
	SysextTimeoutOverride        = "[Service]\nTimeoutStartSec=10\n[Unit]\nJobRunningTimeoutSec=5\n"
	SysextImagePolicyOverride    = "[Service]\nExecStart=systemd-sysext refresh " + SysextDefaultPolicy + "\nExecReload=systemd-sysext refresh " + SysextDefaultPolicy + "\n"
)
