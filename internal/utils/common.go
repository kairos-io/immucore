...
// CheckEfiPartUUID will try to check the /sys/firmware/efi/LoaderDevicePartUUID-4a67b082-0a4c-41cf-b6c7-440b29bb8c4f
...
// This will nil if we are running from a DISK device
// This wil an error when running from a volatile media, like CD or netboot as it cannot infer where it was booted from
...
func CheckEfiPartUUID() error {
	...
	if len(readFile) == 0 || string(readFile) == "" {
		...
		return fmt.Errorf("LoaderDevicePartUUID is empty")
	}
	return nil
}
